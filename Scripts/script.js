...
{   
    ...
    horloge_plateau+=1
    consiel= Math.floor(Math.random() * 1);
    document.getElementById("de").innerHTML=nb;
    document.getElementById("case").innerHTML=joueur1.case_pion;      
    document.getElementById("charge").innerHTML=joueur1.charge; 
...
function deplacement_du_pion(numero_du_de)
{
    ...
    for(i=0;i<4;i++)
    {   if (tableau_descente[i]==joueur1.case_pion)
        {  joueur1.charge-=1;
            joueur1.case_pion=joueur1.case_pion+ numero_du_de;    
            return 
        }
        if (tableau_montee[i]==joueur1.case_pion)
        {   joueur1.charge-=4;
            joueur1.case_pion=joueur1.case_pion+ numero_du_de;
            return  
        }
...
{//TODO verification que ca soit aps les meme numero 
    for(i=0;i<4;i++)
    {
        ...
        document.getElementById("et").innerHTML=tableau_entre_tunnel; 
        document.getElementById("st").innerHTML=tableau_sortie_tunnel; 
        document.getElementById("cd").innerHTML=tableau_descente; 
        document.getElementById("cm").innerHTML=tableau_montee; 
